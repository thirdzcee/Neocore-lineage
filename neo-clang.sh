export KBUILD_BUILD_USER=thirdzcee

DIR=$(pwd)
BUILD="$DIR/build"
OUT="$DIR/zip"
DATE=`date '+%Y-%m-%d'`;
ZIPNAME="lineage-neocore-$DATE-clang.zip"
NPR=`expr $(nproc) + 1`

echo "cleaning build..."
if [ -d "$BUILD" ]; then
rm -rf "$BUILD"
fi
if [ -d "$OUT" ]; then
rm -rf "$OUT/modules"
rm -rf "$OUT/Image.gz-dtb"
fi

echo "setting up build..."
mkdir "$BUILD"
make O="$BUILD" ARCH=arm64 lineageos_h990_defconfig

echo "building kernel..."
time make O="$BUILD" -j$NPR 2>&1 \
ARCH=arm64 \
CLANG_TRIPLE=aarch64-linux-android- \
CXX=/home/ironbuang/android/toolchains/llvm/prebuilt/linux-x86_64/bin/clang++ \
CC=/home/ironbuang/android/toolchains/llvm/prebuilt/linux-x86_64/bin/clang \
CROSS_COMPILE=/home/ironbuang/android/toolchains/aarch64-linux-android-4.9/bin/aarch64-linux-android- |tee ../compile.log

echo "building modules..."
make O="$BUILD" INSTALL_MOD_PATH="." INSTALL_MOD_STRIP=1 modules_install
rm $BUILD/lib/modules/*/build
rm $BUILD/lib/modules/*/source

mkdir -p $OUT/modules
mv "$BUILD/arch/arm64/boot/Image.gz-dtb" "$OUT/Image.gz-dtb"
find "$BUILD/lib/modules/" -name *.ko | xargs -n 1 -I '{}' mv {} "$OUT/modules"
cd zip
#mv modules/exfat.ko modules/texfat.ko
zip -q -r "$ZIPNAME" anykernel.sh META-INF tools modules Image.gz-dtb setfiles.conf ramdisk patch

mv "$ZIPNAME" "/home/ironbuang/android/h990/$ZIPNAME"

rm -rf "$OUT/modules"
rm -rf "$OUT/Image.gz-dtb"

echo "Done !"
